...
$secureApp = strpos(env('APP_URL', ''), 'https://') === 0;
...
if ($secureApp && (empty($_SERVER['HTTPS']) || $_SERVER['HTTPS'] !== 'on')) {
...
if(isset($_SESSION['user']) && $_SESSION['user']['role']==='firma') {
    $subInfo = checkSubscription($pdo, (int)$_SESSION['user']['firm_id']);
    if($subInfo['status']==='expired' && basename($_SERVER['SCRIPT_NAME'])!=='subscription_expired.php') {
        header('Location: /subscription_expired.php?code=E_EXPIRED');
        exit;
    }
    $GLOBALS['subscription_status'] = $subInfo;
}
