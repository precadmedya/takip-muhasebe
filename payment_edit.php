...
try {
    ...
    $rates=cache_get('latest_rates',300); if(!$rates){ $rates=$pdo->query("SELECT * FROM exchange_rates ORDER BY fetched_at DESC LIMIT 1")->fetch(); if($rates) cache_set('latest_rates',$rates); }
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if(!$errors){
            ...
            $old = $pay;
            $pdo->prepare("UPDATE payments SET customer_id=?,product_id=?,service_id=?,extra_item_id=?,currency=?,amount=?,amount_try=?,payment_date=?,notes=? WHERE id=? AND firm_id=?")
                ->execute([$customer_id,$product_id,$service_id,$extra_id,$currency,(float)$amount,$amount_try,$date,$notes?:null,$id,$firmId]);
    ...
    <div class="mb-3">
        ...
        <select name="customer_id" class="form-select">
            ...
            <?php foreach($customers as $c): ?>
                <option value="<?=$c['id']?>" <?php if($pay['customer_id']==$c['id']) echo 'selected'; ?>><?=htmlspecialchars($c['full_name'])?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="mb-3">
        ...
        <select name="product_id" class="form-select">
            ...
            <?php foreach($products as $p): ?>
                <option value="<?=$p['id']?>" <?php if($pay['product_id']==$p['id']) echo 'selected'; ?>><?=htmlspecialchars($p['name'])?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="mb-3">
        ...
        <select name="service_id" class="form-select">
            ...
            <?php foreach($services as $s): ?>
                <option value="<?=$s['id']?>" <?php if($pay['service_id']==$s['id']) echo 'selected'; ?>><?=htmlspecialchars($s['service_name'])?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="mb-3">
        ...
        <select name="extra_item_id" class="form-select">
            ...
            <?php foreach($extras as $e): ?>
                <option value="<?=$e['id']?>" <?php if($pay['extra_item_id']==$e['id']) echo 'selected'; ?>><?=htmlspecialchars($e['title'])?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <div class="row">
        <div class="col-md-4 mb-3">
            ...
            <select name="currency" class="form-select">
                <?php foreach(['TRY','USD','EUR','GBP'] as $cur): ?>
                    <option value="<?=$cur?>" <?php if($pay['currency']==$cur) echo 'selected'; ?>><?=$cur?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label">Tutar</label>
            <input type="number" step="0.01" name="amount" class="form-control" value="<?=htmlspecialchars($pay['amount'])?>" required>
        </div>
        <div class="col-md-4 mb-3">
            <label class="form-label">Tarih</label>
            <input type="date" name="payment_date" class="form-control" value="<?=htmlspecialchars($pay['payment_date'])?>" required>
        </div>
    </div>
    <div class="mb-3">
        <label class="form-label">Notlar</label>
        <textarea name="notes" class="form-control"><?=htmlspecialchars($pay['notes'])?></textarea>
    </div>
...
<?php
    ...
    error_log('[PAYMENT_EDIT] '.$code.' '.$e->getMessage().' @'.$e->getFile().':'.$e->getLine());
