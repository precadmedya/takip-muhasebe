...
if (file_exists($baseFile)) {
    ...
    foreach (array_filter(array_map('trim', preg_split('/;\s*\n/', $sql))) as $stmt) {
        try {
            $pdo->exec($stmt);
            $actions[] = 'Executed: ' . htmlspecialchars(substr($stmt,0,60));
        } catch (Throwable $e) {
            $actions[] = 'Error: ' . $e->getMessage();
        }
    }
}
// Patch services table
try {
    ...
    if(!$hasServiceName && $hasName){
        ...
        $actions[] = 'Renamed name to service_name';
    } else { $actions[] = 'Service name column ok'; }
